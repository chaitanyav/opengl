...
// rule = F → F+F-F--F+F+F
// angle  : 72°
// + turn  left by 72 degrees
// - turn right by 72 degrees
std::string pentadentritelSystemRepresentation() {
...
int main(int argc, char *argv[]) {
  ...
  window = glfwCreateWindow(vidmode->width, vidmode->height, "Pentadentrite", monitor, NULL);
  if(!window) {
    glfwTerminate();
    return -1;
  }
